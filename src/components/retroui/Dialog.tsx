...
interface IDialogBackgroupProps extends HTMLAttributes<HTMLDivElement>, VariantProps<typeof overlayVariants> {}
...
const DialogBackdrop = (inputProps: IDialogBackgroupProps & { ref?: React.Ref<HTMLDivElement> }) => {
  ...
  return <BaseDialog.Backdrop className={cn(overlayVariants({ variant }), className)} ref={ref} {...props} />;
};
...
const DialogContent = (inputProps: IDialogContentProps & { ref?: React.Ref<HTMLDivElement> }) => {
  const { children, size = "auto", className, overlay, ref, ...props } = inputProps;

  return (
    <BaseDialog.Portal>
      <DialogBackdrop {...overlay} />
      <BaseDialog.Popup className={cn(dialogVariants({ size }), className)} ref={ref} {...props}>
...
interface IDialogHeaderProps extends HTMLAttributes<HTMLDivElement>, VariantProps<typeof dialogHeaderVariants> {
  asChild?: boolean;
  closable?: boolean;
}

const DialogHeader = ({ children, className, position, variant, asChild, closable = true, ...props }: IDialogHeaderProps) => {
  return (
    <div className={cn(dialogHeaderVariants({ position, variant }), className)} {...props}>
      {asChild ? children : closable ? <DialogHeaderDefaultLayout>{children}</DialogHeaderDefaultLayout> : <>{children}</>}
    </div>
  );
};
