...
const Avatar = ({ className, ref, ...props }: React.ComponentPropsWithRef<typeof BaseAvatar.Root>) => (
  <BaseAvatar.Root ref={ref} className={cn("relative flex h-14 w-14 border-2 overflow-hidden", className)} {...props} />
);
...
const AvatarFallback = ({ className, ref, ...props }: React.ComponentPropsWithRef<typeof BaseAvatar.Fallback>) => (
  <BaseAvatar.Fallback ref={ref} className={cn("flex h-full w-full items-center justify-center bg-primary text-primary-foreground", className)} {...props} />
);
